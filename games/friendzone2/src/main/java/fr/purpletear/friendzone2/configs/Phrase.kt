...
class Phrase : Parcelable {
    ...
    private val isCondition: Boolean
        get() =
            looksLikeACode() && sentence.matches(("\\[([a-zéA-Z0-9]+)([ ]*)=([ ]*)([a-zéA-Z0-9]+)\\]").toRegex())
    ...
    internal fun formatVars(table: TableOfSymbols) {
        for (`var` in table.getArray(GlobalData.Game.FRIENDZONE2.id)) {
            sentence = sentence.replace("[[" + `var`.n + "]]", `var`.v)
        }
    }

    /**
     * Determines if the current Phrase is a split
     * @return Boolean
     */
    fun isSplit() : Boolean {
        return sentence.replace(" ", "").lowercase() == "[split]"
    }
    ...
    /**
     * Replace smileys to emojis
     */
    fun toEmojis() {
        sentence = sentence.replace(":kissingheart:", String(java.lang.Character.toChars(0x1F618)))
    }
    ...
    fun getVarFromCondition(chapterNumber: Int): Var {
        ...
        val pattern = Pattern.compile("\\[([a-zéA-Z0-9]+)([ ]*)=([ ]*)([a-zéA-Z0-9]+)\\]")
    ...
    companion object {
        ...
        fun determineTypeCode(type: Type): Int {
            return when (type) {
                ...
                Phrase.Type.meTyping -> 11
                Phrase.Type.vocal -> 12
                Phrase.Type.noSignal -> 13
                Type.trophy -> 14
                else -> -1
            }
        }
    ...
    /**
     * Determines if the sentence contains a code
     * @return Boolean
     */
    fun isCode() : Boolean {
        return sentence.replace(" ", "").startsWith("[CODE")
    }

    /**
     * Determines if the sentence is a call to no signal
     * @return Boolean
     */
    fun isNoSignal() : Boolean {
        return sentence.replace(" ", "").lowercase() == "[no_signal]"
    }

    /**
     * Returns the calling code number
     * @return Int
     */
    fun getCode() : Int {
        if(!isCode()) {
            throw java.lang.IllegalStateException("Calling getCode in a non-code Phrase")
        }
