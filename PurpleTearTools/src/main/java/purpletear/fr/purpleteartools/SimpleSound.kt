...
class SimpleSound(val root : String = "") {
    private var player : MediaPlayer? = null
    private var currentSoundName = ""
    private var currentMsPosition : Int = 0

    fun prepareAndPlay(activity : Activity, resource : Int, isLooping : Boolean, at : Int = 0) {
        stop(true)

        try {

            player = MediaPlayer.create(activity, resource)
            currentMsPosition = 0
    ...
    fun prepareAndPlay(activity : Activity, name : String, isLooping : Boolean, at : Int = 0) {
        ...
        try {
            val fd = activity.assets.openFd("$root/sound/$name.mp3")
