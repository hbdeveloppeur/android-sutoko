package com.purpletear.sutoko.auth.data
...
object UserHelper {
    ...
    fun login(
        ...
        onComplete: (
            ...
            resultCode: AccountFormHelper.FormValidationCode,
            token: String?,
            uid: String?
        ) -> Unit
    ) {
        ...
        request(activity, "https://create.sutoko.app/api/login", parameters) { response ->
            ...
            val uid = response.uid?.toString()
            val isSuccess = response.result
            val resultCode = response.error
            if (isSuccess == true) {
                onComplete(true, AccountFormHelper.FormValidationCode.SUCCESS, token, uid)
            } else {
                ...
                onComplete(false, formValidationCode, token, uid)
            }
        }
    }
