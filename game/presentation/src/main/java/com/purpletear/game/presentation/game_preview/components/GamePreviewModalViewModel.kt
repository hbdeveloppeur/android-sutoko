...
class GamePreviewModalViewModel @Inject constructor(
    ...
    fun onAction(action: GamePreviewAction) {
        when (action) {
            GamePreviewAction.OnPlay -> {
                viewModelScope.launch { _playGameEvents.emit(_game.value) }
            }

            GamePreviewAction.OnDownload -> {
                startDownload()
            }

            GamePreviewAction.OnReload -> {
                gameId?.let { loadGame(it) }
            }

            GamePreviewAction.OnUpdateGame -> {
                startDownload()
            }

            GamePreviewAction.OnAbortBuy,
            GamePreviewAction.OnBuy,
            GamePreviewAction.OnBuyConfirm -> {
                Log.d(TAG, "Buy action triggered - emitting dismissEvents")
                viewModelScope.launch { _dismissEvents.emit(Unit) }
            }

            GamePreviewAction.OnRestart -> {
                // TODO:
            }

            GamePreviewAction.OnUpdateApp -> {
                viewModelScope.launch { _openStoreEvents.emit(Unit) }
            }

            GamePreviewAction.OnDelete -> {
            ...
            gameDownloadManager.getDownloadState(gameId).collect { downloadState ->
                when (downloadState) {
                    is GameDownloadState.Downloading -> {
                        _gameState.value =
                            GameState.DownloadingGame(progress = downloadState.progress)
                    }
