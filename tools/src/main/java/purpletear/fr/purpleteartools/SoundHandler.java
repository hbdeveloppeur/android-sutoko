...
public class SoundHandler {
        ...
        for(Sound sound : array){
            if(sound.name.equals(name)){
                try {
                    sound.mp.seekTo(at);
                    sound.mp.start();
                    sound.mp.setOnCompletionListener(mp -> {
                        mp.release();
                        new Handler().post(r);
                    });
                } catch (Exception e) {
    ...
    public SoundHandler generateFromExternalStorage(String filepath, Activity activity, final boolean loop){
        removeIfExist(filepath);
        try {

            MediaPlayer player = new MediaPlayer();
            player.setLooping(loop);
            player.setDataSource(new File(filepath).getPath());
            player.prepare();
    ...
    public SoundHandler generate(String name, Activity activity, final boolean loop){
        removeIfExist(name);
        try {
            AssetFileDescriptor message_audio = activity.getAssets().openFd(path+ File.separator + "sounds/" + name + ".mp3");
            MediaPlayer player = new MediaPlayer();
            player.setLooping(loop);
